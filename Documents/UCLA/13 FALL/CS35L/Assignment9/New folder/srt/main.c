...
float total_color[width][height][3];
...
main( int argc, char **argv )
{
	...
	for (px = 0; px < width; px++)
	{
		...
		{
			printf( "%.0f %.0f %.0f\n",total_color[px][py][0], total_color[px][py][1], total_color[px][py][2]);
		}
		printf( "\n" );
	}
...
void *updateThread(void *arg)
{
	...
	int block=(int)(width/args->nthreads);
	int index;
	int id;
	Vec3 camera_pos;
	set( camera_pos, 0., 0., -4. );
	...
	for (index=block*id;index<(id+1)*block & index<width;++index)
	{
        const double x = pixel_dx * ((double)( index-(width/2) ));
        for( int py=0; py<height; ++py )
        {
            ...
            for( int i=0; i<3; i++)
                total_color[index][py][i] = max( min(scaled_color[i], 255), 0);
	...
	{
		for (index=block*(id+1);index<width;++index)
		{
			const double x = pixel_dx * ((double)( index-(width/2) ));
			for( int py=0; py<height; ++py )
			{
